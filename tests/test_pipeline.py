...
def test_pipeline_no_files(temp_output):
    ...
    with tempfile.TemporaryDirectory() as empty_dir:
        ...
        assert (
            results["error"]
            == "No files discovered in the specified directory. Please ensure the directory contains supported source code files. See https://github.com/MichaelWeed/sakura-sumi#file-type-support for supported types."
        )
...
def test_pipeline_smart_concatenation(temp_codebase, temp_output, capsys):
    ...
    assert (
        "Smart concatenation complete" in out or "Smart concatenation pipeline" in out
    )
    assert results["failure_report"] is None
