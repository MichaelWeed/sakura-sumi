...
def test_ocr_compressor_compress_pdf_with_cache(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        compressor, "_load_model", lambda: setattr(compressor, "model_loaded", True)
    )
...
def test_ocr_compressor_batch_processing(monkeypatch, tmp_path):
    ...
    progress_calls = []
    results = compressor.compress_pdfs_batch(
        pdf_paths,
        progress_callback=lambda path, success: progress_calls.append((path, success)),
    )
