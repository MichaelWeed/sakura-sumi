...
def test_deepseek_insights_processing_time(insights_service):
    ...
    assert (
        "seconds" in insights["processing_time_formatted"]
        or "minutes" in insights["processing_time_formatted"]
    )
...
def test_deepseek_insights_config_without_nesting():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        config = {"compression_ratio": 15, "accuracy": 0.98}
        json.dump(config, f)
        config_path = Path(f.name)
...
def test_deepseek_insights_custom_compression_ratio(temp_config_file):
    ...
    # Expected ~8333 (100000 / 12), rounded up to 9k
