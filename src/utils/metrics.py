...
class CompressionMetrics:
    ...
    def estimate_pdf_tokens(
        self, pdf_size_bytes: int, page_count: Optional[int] = None
    ) -> int:
        """
    ...
    def calculate_metrics(
        ...
        fits_gemini_ocr = (
            total_ocr_tokens <= self.GEMINI_CONTEXT_LIMIT if total_ocr_tokens else None
        )
        ...
        return {
            ...
            "ocr": (
                {
                    ...
                    "token_savings": ocr_token_savings,
                    "token_savings_percent": ocr_token_savings_pct,
                }
                if ocr_stats
                else None
            ),
            "gemini_compatibility": {
                ...
                "pdf_usage_percent": (
                    (total_pdf_tokens / self.GEMINI_CONTEXT_LIMIT * 100)
                    if fits_gemini_pdf
                    else None
                ),
                "ocr_usage_percent": (
                    (total_ocr_tokens / self.GEMINI_CONTEXT_LIMIT * 100)
                    if fits_gemini_ocr and total_ocr_tokens
                    else None
                ),
            },
            "summary": {
    ...
    def _generate_markdown_report(self, metrics: Dict[str, Any]) -> str:
        ...
        if gemini["pdf_usage_percent"]:
            lines.append(
                f"- **PDF Usage**: {gemini['pdf_usage_percent']:.1f}% of context window"
            )

        if gemini.get("fits_ocr") is not None:
            lines.append(
                f"- **OCR Fits**: {'✅ Yes' if gemini['fits_ocr'] else '❌ No'}"
            )
