...
class DeepSeekInsightsService:
    ...
    def _load_config(self, config_path: Optional[Path] = None) -> Dict[str, Any]:
        ...
        if config_path is None:
            ...
            config_path = (
                Path(__file__).parent.parent.parent / "configs" / "deepseek_ocr.json"
            )
    ...
    def calculate_insights(self, file_count: int, pre_tokens: int) -> Dict[str, Any]:
        ...
        processing_time_seconds = (
            estimated_pages * self.config["avg_processing_time_per_page_seconds"]
        )
        ...
        capacity_percentage = (
            (estimated_pages / pages_per_day) * 100 if pages_per_day > 0 else 0
        )
        ...
        vision_tokens_estimate = (
            math.ceil(compressed / 1000) * 1000 if compressed > 0 else 1
        )
