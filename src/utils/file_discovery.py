...
class FileDiscovery:
    ...
    DEFAULT_EXCLUSIONS = {
        ...
        ".vscode",
        ".vs",
        "android",  # Android build files (for the sample codebase)
    ...
    def _should_exclude(self, path: Path) -> bool:
        ...
        for exclusion in self.exclusions:
            ...
            if "*" in exclusion or "?" in exclusion:
                ...
                if fnmatch.fnmatch(relative_str, exclusion) or fnmatch.fnmatch(
                    path.name, exclusion
                ):
                    return True
                ...
                if (
                    exclusion.rstrip("/") in relative_str
                    or exclusion.rstrip("/") in path_str
                ):
    ...
    def generate_inventory_report(self) -> Dict:
        ...
        source_basename = (
            self.source_dir.name if self.source_dir.name else Path(self.source_dir).name
        )
        return {
