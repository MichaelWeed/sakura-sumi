...
def main():
    ...
    parser.add_argument(
        "source_dir", type=str, help="Source codebase directory to compress"
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--exclude", nargs="+", default=[], help="Additional exclusion patterns"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--generate-charts", action="store_true", help="Generate visualization charts"
    )
    parser.add_argument(
    ...
    if not source_path.exists():
        ...
        print(
            "   💡 Tip: Use an absolute path or ensure you're in the correct working directory."
        )
        sys.exit(1)

    if not source_path.is_dir():
        print(f"\n✗ Error: Path is not a directory: {args.source_dir}")
        print("   💡 Tip: The specified path points to a file, not a directory.")
        print("   💡 Tip: Provide the directory containing your source code files.")
        sys.exit(1)
    ...
    # A home directory is either the current user's own, or a direct sibling
    # of it (another account's home on the same machine: /Users/<name>,
    # /home/<name>, C:\Users\<name>). Derived from Path.home() rather than a
    # hardcoded "/Users" so it holds on Linux and Windows too.
    home = Path.home()
    if source_path == home or source_path.parent == home.parent:
        error_msg = (
    ...
    try:
        ...
        if results["success"]:
            ...
            if args.generate_charts:
                charts = create_visualizations(
                    results["metrics"], output_dir / "charts"
                )
                if charts:
                    print(
                        f"✓ Generated {len(charts)} visualization charts in: {output_dir / 'charts'}"
                    )
            ...
            print(
                f"   💡 Tip: Check the output directory for detailed error logs: {output_dir}"
            )
            print(
                "   💡 Tip: Run with --verbose flag for more detailed error information."
            )
            print(
                "   💡 Tip: Check file permissions and ensure you have write access to the output directory."
            )
            sys.exit(1)
        ...
        print("   💡 Tip: Run with --verbose flag to see the full stack trace.")
        print(
            "   💡 Tip: Check that all dependencies are installed: pip install -r requirements.txt"
        )
        print("   💡 Tip: Ensure you have sufficient disk space and file permissions.")
        import traceback
