...
class CompressionPipeline:
    ...
    def __init__(
        ...
        self.checkpoint_file = (
            ...
            if checkpoint_file
            else self.output_dir / ".compression_checkpoint.json"
        )
        ...
        self.converter = PDFConverter(
            str(self.output_dir), hybrid_mode=hybrid_mode
        )  # HYBRID_MODE_START
    ...
    def _process_file_batch(
        self, files: List[FileInfo], progress_callback: Optional[Callable] = None
    ...
    def _process_parallel(
        self, files: List[FileInfo], verbose: bool = True
    ) -> tuple[int, int]:
    ...
    def run(self, verbose: bool = True) -> dict:
        ...
        if not files:
            ...
            unsupported = discovery_stats.get("statistics", {}).get(
                "unsupported_files", {}
            )
            total_scanned = discovery_stats.get("statistics", {}).get(
                "total_scanned", 0
            )
            ...
            if verbose:
                ...
                if unsupported:
                    ...
                    print(
                        f"   Unsupported file types: {', '.join(sorted(unsupported.keys()))}"
                    )
                    print(
                        "   💡 Tip: Sakura Sumi only processes text-based source code files."
                    )
                    print(
                        "      See README.md for a complete list of supported file types."
                    )
            ...
            self._emit_telemetry(
                ...
                failure_results,
                {
                    ...
                    "unsupported_types": (
                        list(unsupported.keys()) if unsupported else []
                    ),
                },
            )
        ...
        if self.parallel:
            ...
            if files_to_process:
                ...
                while batch_start < len(files_to_process) and not self.cancelled:
                    batch_end = min(
                        batch_start + self.batch_size, len(files_to_process)
                    )
                    batch = files_to_process[batch_start:batch_end]

                    batch_converted, batch_failed, batch_errors = (
                        self._process_file_batch(
                            batch, lambda f, success: pbar.update(1) if pbar else None
                        )
                    )
        ...
        # Calculate metrics
        original_files = [{"size": f.size, "path": f.relative_path} for f in files]
        ...
        files_already_processed = (
            len(self.processed_files) - converted_count if self.resume else 0
        )
        ...
        results = {
            ...
            "source_directory": self._sanitize_path(
                self.source_dir
            ),  # Sanitized: basename only
            "output_directory": self._sanitize_path(
                self.output_dir
            ),  # Sanitized: basename only
    ...
    def _print_metrics_summary(self, metrics: Dict[str, Any]):
        ...
        print(
            f"Original: {orig['estimated_tokens']:,} tokens ({self._format_size(orig['total_size_bytes'])})"
        )
        print(
            f"PDF:      {pdf['estimated_tokens']:,} tokens ({self._format_size(pdf['total_size_bytes'])})"
        )
        ...
        if metrics.get("ocr"):
            ...
            if gemini.get("ocr_usage_percent"):
                print(
                    f"  OCR Usage: {gemini['ocr_usage_percent']:.1f}% of context window"
                )
    ...
    def _print_summary(self, results: dict):
        ...
        if results["summary"]["files_already_processed"] > 0:
            print(
                f"Files Already Processed: {results['summary']['files_already_processed']}"
            )
        print("\nSize Statistics:")
        print(
            f"  Original: {self._format_size(results['summary']['total_size_original_bytes'])}"
        )
        print(
            f"  PDF:      {self._format_size(results['summary']['total_size_pdf_bytes'])}"
        )
    ...
    def run_smart_concatenation(
        ...
        if verbose:
            ...
            print(
                f"Total directories found: {len(set(str(Path(f.relative_path).parent) if len(Path(f.relative_path).parts) > 1 else '' for f in files))}"
            )
            ...
            for group in pdf_groups:
                ...
                if len(group.files) > 0:
                    print(
                        f"    Sample files: {', '.join([f.relative_path for f in group.files[:3]])}"
                    )
    ...
    def _prepare_conversion_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            **{
                ...
                if k
                not in {"total_size_original", "total_size_pdf", "compression_ratio"}
            },
        }
