...
class SmartConcatenationEngine:
    ...
    def __init__(
        ...
        self.key_folders = (
            set(key_folders) if key_folders else self.DEFAULT_KEY_FOLDERS.copy()
        )

    def build_directory_tree(
        self, files: List[FileInfo]
    ) -> Tuple[Dict[str, List[FileInfo]], List[FileInfo]]:
        """
    ...
    def calculate_directory_priority(
        self, dir_path: str, file_count: int, total_size: int
    ) -> int:
        """
    ...
    def roll_up_directories(
        self, dir_tree: Dict[str, List[FileInfo]]
    ...
    def group_files(self, files: List[FileInfo]) -> List[PDFGroup]:
        """
        ...
        if len(dir_tree) <= self.max_pdfs:
            ...
            for dir_path, dir_files in dir_tree.items():
                ...
                groups.append(
                    PDFGroup(
                        ...
                        priority=self.calculate_directory_priority(
                            dir_path, len(dir_files), total_size
                        ),
                    )
                )
        else:
            ...
            for folder_name, subdirs in key_folder_groups.items():
                ...
                slots_for_key_folders = (
                    self.max_pdfs - len(other_dirs) - 1
                )  # -1 for potential root/misc
                ...
                if len(subdirs) <= slots_for_key_folders:
                    ...
                    groups.append(
                        PDFGroup(
                            ...
                            priority=self.calculate_directory_priority(
                                folder_name, len(all_files), total_size
                            ),
                        )
                    )
    ...
    def _apply_misc_bucket(self, groups: List[PDFGroup]) -> List[PDFGroup]:
        ...
        if misc_files:
            top_groups.append(
                PDFGroup(
                    name="misc.pdf", files=misc_files, directory_path="misc", priority=0
                )
            )
