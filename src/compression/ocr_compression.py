...
class OCRCompressor:
    ...
    def __init__(self, mode: str = "small", cache_dir: Optional[str] = None):
        ...
        if mode not in self.COMPRESSION_MODES:
            raise ValueError(
                f"Invalid mode: {mode}. Choose from {list(self.COMPRESSION_MODES.keys())}"
            )
    ...
    def compress_pdf(self, pdf_path: Path) -> Optional[Path]:
        ...
        try:
            ...
            compressed_content += (
                f"Expected Accuracy: {self.config['accuracy']*100:.0f}%\n\n"
            )
    ...
    def compress_pdfs_batch(
        self, pdf_paths: List[Path], progress_callback: Optional[callable] = None
    ) -> Dict[str, Path]:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.stats["total_output_size"] > 0:
            compression_ratio = (
                self.stats["total_input_size"] / self.stats["total_output_size"]
            )
...
def create_ocr_compressor(
    mode: str = "small", cache_dir: Optional[str] = None
) -> Optional[OCRCompressor]:
    """
