...
class PDFConverter:
    ...
    def convert_file(self, file_info: FileInfo) -> Optional[Path]:
        ...
        try:
            ...
            is_first_file = (
                not self.translation_dict_global
            )  # Track if this is the first file processed
            if self.hybrid_mode:
                processed_content, translation_dict = preprocess_code(
                    content, mode="hybrid"
                )
                ...
                if is_first_file and self.translation_dict_global:
                    ...
                    print(
                        f"[HYBRID] Prepended translation dict to {file_info.relative_path} ({len(self.translation_dict_global)} mappings)"
                    )
                elif translation_dict:
                    print(
                        f"[HYBRID] Processed {file_info.relative_path}: {len(translation_dict)} symbol replacements"
                    )
            # HYBRID_MODE_END
            ...
            if pdf_path and pdf_path.exists():
                ...
                self.conversion_stats["errors"].append(
                    {"file": file_info.relative_path, "error": "PDF generation failed"}
                )
                return None
            ...
            self.conversion_stats["failed"] += 1
            self.conversion_stats["errors"].append(
                {"file": file_info.relative_path, "error": str(e)}
            )
            return None
    ...
    def _read_file(self, file_info: FileInfo) -> Optional[str]:
        ...
        try:
            with open(
                file_info.path, "r", encoding=file_info.encoding, errors="replace"
            ) as f:
                return f.read()
            ...
            for encoding in ["latin-1", "cp1252", "iso-8859-1"]:
                try:
                    with open(
                        file_info.path, "r", encoding=encoding, errors="replace"
                    ) as f:
                        return f.read()
            ...
            self.conversion_stats["errors"].append(
                {"file": file_info.relative_path, "error": f"File read error: {str(e)}"}
            )
            return None
    ...
    def _format_structured_data(self, content: str, file_type: str) -> str:
        ...
        try:
            if file_type == "json":
                ...
                return yaml.dump(
                    data, indent=2, default_flow_style=False, allow_unicode=True
                )
        except Exception:
    ...
    def _generate_pdf(self, file_info: FileInfo, content: str) -> Optional[Path]:
        ...
        try:
            ...
            story.append(
                Paragraph(f"<b>File:</b> {file_info.relative_path}", header_style)
            )
            story.append(
                Paragraph(
    ...
    def _generate_pdf_simple(
        self, file_info: FileInfo, content: str, pdf_path: Path
    ) -> Path:
        ...
        c.drawString(
            self.MARGIN,
            y_pos,
            f"Type: {file_info.file_type} | Size: {file_info.size} bytes",
        )
    ...
    def concatenate_files_to_pdf(
        ...
        try:
            ...
            is_first_file = (
                True  # HYBRID_MODE_START - track first file for dict prepending
            )
            ...
            self.conversion_stats["errors"].append(
                {"files": [f.relative_path for f in files], "error": str(e)}
            )
            return None
