...
if not os.environ.get("FLASK_SECRET_KEY"):
    ...
    warnings.warn(
        "FLASK_SECRET_KEY not set. Using ephemeral key. Sessions will not persist across restarts."
    )
    ...
    if prompt_payload:
        ...
        try:
            ...
            try:
                ...
                post_estimation = token_service.estimate_post_compression(
                    pre_estimation["total_tokens"]
                )
                recommendation = token_service.get_recommendation(
                    pre_estimation["total_tokens"]
                )

                # Calculate DeepSeek insights
                insights_service = DeepSeekInsightsService()
                insights = insights_service.calculate_insights(
                    len(files), pre_estimation["total_tokens"]
                )
                ...
                return jsonify(
                    {
                        ...
                        "technical_details": insights_service.get_technical_details(
                            insights
                        ),
                    }
                )
...
def compress():
    ...
    if is_prompt_mode:
        ...
        if not output_dir:
            ...
            output_dir = str(
                default_root / f"session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
    else:
        # Strip quotes and whitespace
        source_dir = source_dir.strip().strip("'\"")
    ...
    job = {
        "id": job_id,
        "status": "queued",
        "source_dir": sanitize_path_for_display(
            source_label
        ),  # Sanitized: basename only for display
...
def run_compression_job(
    ...
    try:
        ...
        if hybrid_mode:
            print("[HYBRID] Hybrid mode enabled - preprocessing code for OCR fidelity")
        ...
        if cleanup_paths:
            for path in cleanup_paths:
                try:
                    ...
                    print(
                        f"Warning: Could not clean prompt workspace {path}: {cleanup_error}"
                    )
